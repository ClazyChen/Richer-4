            ...
            MoveInHandTriggerList.Add((PPlayer Player, PCard Card) => {
                return new PTrigger(CardName) {
                    ...
                    Condition = (PGame Game) => {
                        ...
                        return Player.Equals(InjureTag.ToPlayer) && InjureTag.FromPlayer != null && InjureTag.Injure > 0 && Game.Map.BlockList.Find((PBlock Block) => Player.Equals(Block.Lord) && Block.HouseNumber > 0) != null;
                    },
                    AICondition = (PGame Game) => {
                        PInjureTag InjureTag = Game.TagManager.FindPeekTag<PInjureTag>(PInjureTag.TagName);
                        return Player.Money <= InjureTag.Injure || (InjureTag.Injure >= 1000 + PAiMapAnalyzer.MaxHouseValue(Game, Player) && InjureTag.FromPlayer.TeamIndex != Player.TeamIndex);
                    },
                    Effect = MakeNormalEffect(Player, Card, AIEmitTargets, AIEmitTargets,
                        (PGame Game, PPlayer User, PPlayer Target) => {
                            ...
                            if (Game.Map.BlockList.Find((PBlock Block) => User.Equals(Block.Lord) && Block.HouseNumber > 0) != null) {
                                PBlock TargetBlock = null;
                                if (Target.IsAI) {
                                    TargetBlock = PAiMapAnalyzer.MaxValueHouse(Game, User);
                                } else {
